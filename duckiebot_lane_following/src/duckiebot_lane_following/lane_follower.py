...
class LaneFollower:
	...
	# Bounding boxes
	self.bounding_boxes = rospy.Subscriber('/darknet_ros/bounding_boxes', BoundingBoxes, self.bb_callback)
	...
	global no_of_objects
	no_of_objects = count_msg.count
	

    def bb_callback(self, bb):
	global box_class
	box_class = bb.bounding_boxes[0].Class
	...
	global v
	omega = -0.75 * y + -1.25 * phi  # Proportional only
	#print('v = ', v)
	#print('omega = ', omega)
	...
	if box_class == "Yellow light":
	    self.cmd_vel_pub.publish(cmd_vel)
	    global v
	    v = 0.07
	    print(v)

	if box_class == "Green light":
	    self.cmd_vel_pub.publish(cmd_vel)
	    global v
	    v = 0.2
	    print(v)
